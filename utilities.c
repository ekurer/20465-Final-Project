...
{
    char *modified = (char *) malloc(strlen(original) + MAX_FILE_EXTENSION);
    if(modified == NULL)
    {
...
/* simply checks if a word is a label declaration assuming label declaration ends with a ':' */
...
/* simply checks if an operand supposed to be a label */
boolean is_operand_label(char *token) {
    return !(is_register(token) || token[0] == '#');
}
...
int which_directive(char *token)
{
    ...
    return which_index(token, directives, NUM_DIRECTIVES);
}

/* Check if a token matches a command name */
int which_command(char *token)
{
    return which_index(token, commands, NUM_COMMANDS);
}
...
/* This functions returns 1 if there's an error (= global variable err has changed) */
int is_error()
{
    return err != NO_ERROR;
}
...
char *next_token_string(char *dest, char *line)
{
    ...
    line = next_list_token_string(dest, line);
    if(*dest != '"') return line;
    while(!end_of_line(line) && dest[strlen(dest) - 1] != '"')
    {
        line = next_list_token_string(temp, line);
